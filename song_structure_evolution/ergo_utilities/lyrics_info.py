...
def setup():
    ...
    return lyricsgenius.Genius(credentials['genius']['client_access_token'], retries=3)
...
def is_artist_on_track(artist, track):
    ...
    if track:
        ...
        if fuzz.partial_ratio(str(artists_on_song), artist.lower()) > 50:        
            return True
        else:
            return False
    else:
...
def download_song_lyrics(folder, song_name, artist, song_id):
    ...
    # Check if there are already songs downloaded
    song_lyric = genius.search_song(song_name, artist)

    if not is_artist_on_track(artist, song_lyric):
        print("Não achou")

        song_adj = clean_song_name(song_name)
        artist_adj = artist.split(',')[0].split('and')[0].strip()
        song_lyric = genius.search_song(song_adj, artist_adj)
